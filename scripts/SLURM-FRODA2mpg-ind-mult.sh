...
echo "python3 ../pdb2movie/generate_video_serial.py \$execdir --modes $modes --ecuts $ecuts --res 1920 1080 $options >& ../$pdb-`echo $modes | sed "s/ /-/g"`_`echo $ecuts | sed "s/ /-/g"`.log"
python3 ../pdb2movie/generate_video_serial.py \$execdir --modes $modes --ecuts $ecuts --res 1920 1080 $options >& ../$pdb-`echo $modes | sed "s/ /-/g"`_`echo $ecuts | sed "s/ /-/g"`.log 
